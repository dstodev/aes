...
static const uint8_t inverse_sbox[16][16] = {
...
static int gfmult(int a, int b);
static const uint8_t mix_con[4][4] = {{2, 3, 1, 1}, {1, 2, 3, 1}, {1, 1, 2, 3}, {3, 1, 1, 2}};
static const uint8_t inv_mix_con[4][4] = {{14, 11, 13, 9}, {9, 14, 11, 13}, {13, 9, 14, 11}, {11, 13, 9, 14}};
...
static void invShiftRows(aes_data_t * data);
...
aes_cipher_t encrypt(const char * message, const char * key)
{
	...
	if (message && key) {
		...
		initData(&data, (uint8_t *) message, strlen(message), key);
...
char * decrypt(aes_cipher_t cipher, const char * key)
{
	aes_data_t data;
	char * result = 0;

	if (cipher.cipher && cipher.cipherLen && key) {
		// Initialize data struct
...
static void initData(aes_data_t * data, const uint8_t * message, size_t messageLen, const char * key)
{
	...
	if (data && message && key) {
		...
		data->numBlocks = (size_t) ceil(messageLen / (double) AES_BLOCK_LEN);
		...
		memcpy(data->blocks, message, messageLen);
...
static void invSubBytes(aes_data_t * data)
{
	...
	if (data) {
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				byte = data->state[i][j];
				byte = inverse_sbox[(byte & 0xF0) >> 4][byte & 0x0F];
				data->state[i][j] = byte;
			}
		}
	}
...
static void mixColumns(aes_data_t * data)
{
	...
	if (data) {
		for (int x = 0; x < 4; ++x) {
			for (int y = 0; y < 4; ++y) {
				for (int z = 0; z < 4; ++z) {
					state_copy[x][y] ^= gfmult(mix_con[x][z], data->state[z][y]);
				}
			}
		}
		memcpy(&data->state, &state_copy, sizeof(data->state));
	}
}
...
static void invMixColumns(aes_data_t * data)
{
	...
	if (data) {
		for (int x = 0; x < 4; ++x) {
			for (int y = 0; y < 4; ++y) {
				for (int z = 0; z < 4; ++z) {
					state_copy[x][y] ^= gfmult(inv_mix_con[x][z], data->state[z][y]);
				}
			}
		}
		memcpy(&data->state, &state_copy, sizeof(data->state));
